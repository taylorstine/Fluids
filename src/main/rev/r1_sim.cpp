...
void set_boundaries(int * bnd, int * vel){
  for(int i =0; i < 4; i++){
    boundaries[i] = bnd[i];
    wall_velocity[i] = vel[i];
  }
}
...
void set_bnd(int N, int b, float *x, int *bnd){
  ...
  for(i = 1; i <= N; i++){
    ...
    case U_BND:
      ...
      case INFLOW: x[AT(N+1,i)] = -.5 - x[AT(N,i)]; break;
      }
      switch(boundaries[BOTTOM]){
      case NO_SLIP: x[AT(i,0)] = wall_velocity[BOTTOM]; break;
      }
      switch(boundaries[TOP]){
      case NO_SLIP: x[AT(i,N+1)] = wall_velocity[TOP];break;//x[AT(i,N)]; break;
      }
      break;
  ...
  FOR_EACH_CELL
	...
	switch(b){
	case U_BND:
	  x[AT(i,j+idx)] = x[AT(i,j+idx)];
	  x[AT(i+CUBE_SIZE, j+idx)] = x[AT(i+CUBE_SIZE, j+idx)];
	  x[AT(i+idx,j)] = -x[AT(i+idx,j)];//0.0;
	  x[AT(i+idx,j+CUBE_SIZE)] = -x[AT(i+idx,j+CUBE_SIZE)];//0.0;
	  break;
	case V_BND:
	  x[AT(i,j+idx)] = -x[AT(i,j+idx)];
	  x[AT(i+CUBE_SIZE,j+idx)] =  -x[AT(i+CUBE_SIZE,j+idx)];
	  x[AT(i+idx,j)] = x[AT(i+idx, j)];
	  x[AT(i+idx,j+CUBE_SIZE)] = x[AT(i+idx, j+CUBE_SIZE)];
	  break;
	default:
